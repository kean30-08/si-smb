                ...
                <tr onclick="window.location='{{ route('pengajar.show', $pengajar->id) }}'"
                    ...
                    <td class="block md:table-cell py-3 md:py-4 px-2 md:px-6 text-right md:text-center text-blue-600 font-medium mt-2 md:mt-0 pt-3 md:pt-4"
                        ...
                        <div class="flex items-center justify-end md:justify-center gap-4">
                            ...
                            {{-- LOGIKA AKSES EDIT & HAPUS --}}
                            @php
                                $isTargetProtected = $pengajar->user_id == 1 || $pengajar->jabatan_id == 2;
                                $canManage = auth()->user()->isAdmin() || !$isTargetProtected;
                            @endphp
                            ...
                            @if ($canManage)
                                ...
                                @if ($isAdmin)
                                    ...
                                    {{-- Tombol Hapus (Dilindungi proteksi ganda agar admin tidak bisa hapus diri sendiri/kepsek) --}}
                                    @if (
                                        !(
                                            $pengajar->user_id == 1 ||
                                    ...
                                    @endif
                                @else
                                    {{-- Ikon gembok untuk memberi tahu pengajar biasa bahwa akses terbatas --}}
                                    <div class="text-gray-300 cursor-not-allowed" title="Akses edit/hapus terbatas">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round">
