...
<body>
    ...
    @if($agendas->count() > 0)
        @php
            ...
            // Mencegah division by zero, dibulatkan ke bilangan bulat terdekat
            $rata_rata = $total_kegiatan > 0 ? round($total_kehadiran / $total_kegiatan) : 0;
            
            // Mencari kegiatan dengan kehadiran tertinggi & terendah menggunakan sortBy
        ...
        <div class="summary-box">
            ...
            <ul class="summary-list">
                ...
                <li>
                    ...
                    {{ $kegiatan_tertinggi->nama_kegiatan }} pada {{ \Carbon\Carbon::parse($kegiatan_tertinggi->tanggal)->translatedFormat('d F Y') }} 
                    ({{ $kegiatan_tertinggi->jumlah_hadir }} Siswa)
                </li>
                <li>
                    <strong>Kehadiran Terendah:</strong> 
                    {{ $kegiatan_terendah->nama_kegiatan }} pada {{ \Carbon\Carbon::parse($kegiatan_terendah->tanggal)->translatedFormat('d F Y') }} 
                    ({{ $kegiatan_terendah->jumlah_hadir }} Siswa)
                </li>
            </ul>

            <p class="summary-paragraph">
                <strong>Kesimpulan:</strong> Berdasarkan data kegiatan pada periode {{ \Carbon\Carbon::parse($mulai)->translatedFormat('d F Y') }} sampai {{ \Carbon\Carbon::parse($selesai)->translatedFormat('d F Y') }}, tercatat sebanyak <strong>{{ $total_kegiatan }} kegiatan</strong> yang telah dilaksanakan. Total akumulasi kehadiran siswa pada seluruh kegiatan mencapai <strong>{{ $total_kehadiran }} kehadiran</strong>, dengan rata-rata <strong>{{ $rata_rata }} siswa hadir per kegiatan</strong>. 
                Tingkat partisipasi tertinggi dicapai pada kegiatan <strong>{{ $kegiatan_tertinggi->nama_kegiatan }}</strong> ({{ \Carbon\Carbon::parse($kegiatan_tertinggi->tanggal)->translatedFormat('d F Y') }}) dengan jumlah <strong>{{ $kegiatan_tertinggi->jumlah_hadir }} siswa</strong>, sedangkan kehadiran paling minim tercatat pada kegiatan <strong>{{ $kegiatan_terendah->nama_kegiatan }}</strong> ({{ \Carbon\Carbon::parse($kegiatan_terendah->tanggal)->translatedFormat('d F Y') }}) dengan jumlah <strong>{{ $kegiatan_terendah->jumlah_hadir }} siswa</strong>.
            </p>
        </div>
    @endif
