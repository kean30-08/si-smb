    ...
    <table class="w-full text-sm text-left text-gray-500">
        ...
        <tbody class="block md:table-row-group">
            @forelse ($siswas as $index => $siswa)
                <tr onclick="window.location='{{ route('siswa.show', $siswa->id) }}'"
                        ...
                        <div class="flex items-center justify-between md:justify-center">
                            ...
                            <div>
                                @if ($siswa->status == 'aktif')
                                    <span
                                        class="px-2 py-1 font-semibold text-green-700 bg-green-100 rounded-full text-[10px] md:text-xs">Aktif</span>
                                @elseif ($siswa->status == 'tidak aktif')
                                    <span
                                        class="px-2 py-1 font-semibold text-red-700 bg-red-100 rounded-full text-[10px] md:text-xs">Tidak
                                        Aktif</span>
                                @elseif ($siswa->status == 'lulus')
                                    <span
                                        class="px-2 py-1 font-semibold text-blue-700 bg-blue-100 rounded-full text-[10px] md:text-xs">Lulus</span>
                                @endif
                            </div>
                        </div>
                    </td>
