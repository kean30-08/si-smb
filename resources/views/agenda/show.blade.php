    ...
    <x-slot name="header">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Rundown Kegiatan: ') }} <br class="lg:hidden"> {{ \Carbon\Carbon::parse($tanggal)->translatedFormat('d F Y') }}
            </h2>
            ...
            <div class="w-full lg:w-auto flex flex-col sm:flex-row flex-wrap gap-2">
                ...
                {{-- Tombol Download PDF --}}
                <a href="{{ route('agenda.download', $tanggal) }}" class="w-full sm:w-auto justify-center bg-teal-600 hover:bg-teal-800 text-white font-bold py-2 px-4 rounded flex items-center shadow transition" title="Download PDF untuk dibagikan via WhatsApp/Manual">
                ...
                <form action="{{ route('agenda.broadcast', $tanggal) }}" method="POST" onsubmit="return confirm('Kirim jadwal rundown tanggal ini ke SEMUA email orang tua yang terdaftar?');" class="w-full sm:w-auto m-0">
                    @csrf
                    <button type="submit" class="w-full sm:w-auto justify-center bg-indigo-600 hover:bg-indigo-800 text-white font-bold py-2 px-4 rounded flex items-center shadow transition">
                ...
                <a href="{{ route('agenda.createDetail', $tanggal) }}" class="w-full sm:w-auto justify-center bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded flex items-center shadow transition">
                    + Tambah Acara
                </a>

                {{-- Tombol Kembali --}}
                <a href="{{ route('agenda.index') }}" class="w-full sm:w-auto justify-center bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded flex items-center shadow transition">
                    &larr; Kembali
                </a>
            </div>
        </div>
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody class="block md:table-row-group">
                                ...
                                <tr class="block md:table-row bg-white border border-gray-200 md:border-0 md:border-b hover:bg-gray-50 transition mb-4 md:mb-0 rounded-lg md:rounded-none shadow-sm md:shadow-none p-4 md:p-0">
                                    ...
                                    {{-- WAKTU --}}
                                    <td class="block md:table-cell py-2 md:py-4 px-2 md:px-6 border-b md:border-none border-dashed border-gray-200 mb-3 md:mb-0 pb-3 md:pb-4 font-bold text-gray-800">
                                        <span class="md:hidden text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Pukul</span>
                                    ...
                                    <td class="block md:table-cell py-2 md:py-4 px-2 md:px-6 mb-2 md:mb-0">
                                        <div class="flex items-center justify-between md:justify-start">
                                            <span class="md:hidden text-xs font-bold text-gray-500 uppercase tracking-wider">Status Acara</span>
                                            
                                            @if ($agenda->status == 'akan datang')
                                                <span class="px-2 py-1 font-semibold text-blue-700 bg-blue-100 rounded-full text-[10px] md:text-xs">Akan Datang</span>
                                            @elseif ($agenda->status == 'sedang berlangsung')
                                                <span class="px-2 py-1 font-semibold text-yellow-700 bg-yellow-100 rounded-full text-[10px] md:text-xs">Berlangsung</span>
                                            @elseif ($agenda->status == 'selesai')
                                                <span class="px-2 py-1 font-semibold text-green-700 bg-green-100 rounded-full text-[10px] md:text-xs">Selesai</span>
                                            @elseif ($agenda->status == 'batal')
                                                <span class="px-2 py-1 font-semibold text-red-700 bg-red-100 rounded-full text-[10px] md:text-xs">Batal</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="block md:table-cell py-3 md:py-4 px-2 md:px-6 text-right md:text-center mt-2 md:mt-0 border-t md:border-none border-gray-50 pt-3 md:pt-4">
                                        <div class="flex justify-end md:justify-center space-x-5">
                                            ...
                                            <form action="{{ route('agenda.destroy', $agenda->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus agenda ini?');" class="m-0">
                                                @csrf
                                                @method('DELETE')
