                    ...
                    <form action="{{ route('materi.update', $materi->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="judul" value="{{ old('judul', $materi->judul) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            </div>
                            ...
                            <div class="p-4 border border-dashed border-gray-300 rounded-md bg-gray-50">
                                <label class="block font-bold text-sm text-gray-800 mb-2">Update File Materi (Opsional)</label>
                                
                                {{-- Info File Lama --}}
                                @if($materi->file_materi)
                                    <div class="mb-3 p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-800 flex items-center justify-between">
                                        <span>File saat ini sudah tersedia.</span>
                                        <a href="{{ asset('storage/' . $materi->file_materi) }}" target="_blank" class="font-bold underline">Lihat File Lama</a>
                                    </div>
                                    <p class="text-xs text-orange-600 mb-3">*Abaikan input di bawah jika tidak ingin mengganti file lama.</p>
                                @endif
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('materi.index') }}" class="text-gray-600 underline mr-4 hover:text-gray-900">Batal</a>
